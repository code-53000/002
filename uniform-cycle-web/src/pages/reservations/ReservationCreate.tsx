...
import { reservationService, StoreReservationRequest } from '../../services/reservation'
...
const ReservationCreate: React.FC = () => {
  ...
  const { loading, error, execute } = useApi(reservationService.createReservation)
  ...
  const updateItem = (index: number, field: keyof ReservationItemForm, value: string) => {
  ...
  const validate = (): boolean => {
    ...
    const nameError = validateRequired(employeeName, '员工姓名')
    if (nameError) newErrors.employee_name = nameError

    const deptError = validateRequired(employeeDepartment, '部门')
    if (deptError) newErrors.employee_department = deptError

    const dateError = validateRequired(pickupDate, '领取日期')
    if (dateError) newErrors.pickup_date = dateError
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const data: StoreReservationRequest = {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center space-x-4">
        ...
        <h1 className="text-2xl font-bold text-gray-900">新建预约领取</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              ...
              <input
                ...
                placeholder="请输入员工姓名"
                className={`input ${errors.employee_name ? 'border-red-500' : ''}`}
              />
            ...
            <div>
              ...
              <input
                ...
                placeholder="请输入部门"
                className={`input ${errors.employee_department ? 'border-red-500' : ''}`}
              />
              {errors.employee_department && (
                <p className="mt-1 text-sm text-red-600">{errors.employee_department}</p>
              )}
            </div>
            <div>
              <label className="label">领取日期</label>
              <input
                type="date"
                value={pickupDate}
                onChange={(e) => setPickupDate(e.target.value)}
                className={`input ${errors.pickup_date ? 'border-red-500' : ''}`}
              />
              {errors.pickup_date && (
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-gray-900">预约明细</h2>
            <button
              type="button"
              onClick={addItem}
